...
export class CreateClientUseCase {
  ...
  async execute(command: CreateClientCommand): Promise<Result<ClientCreated>> {
    ...
    if (name.length === 0) {
      return Result.failure(UseCaseError.validation('CLIENT_NAME_REQUIRED', 'Client name must not be empty.'));
    }
    if (code.length === 0) {
      return Result.failure(UseCaseError.validation('CLIENT_CODE_REQUIRED', 'Client code must not be empty.'));
    }
