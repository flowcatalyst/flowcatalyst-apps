...
async function syncEventSchemas(client: FlowCatalystClient): Promise<void> {
  const listResult = await client
    .eventTypes()
    .list({ application: PINPOINT_APPLICATION_CODE });
  if (listResult.isErr()) {
  ...
  for (const event of pinpointEventTypes) {
    ...
    if (pushResult.isErr()) {
      throw new Error(
        `Failed to push schema for ${event.code}: ${String(pushResult.error)}`,
      );
    }
  ...
  console.log(
    `[schema-sync] done — pushed=${pushed} skipped=${skipped} missing=${missing}`,
  );
}
