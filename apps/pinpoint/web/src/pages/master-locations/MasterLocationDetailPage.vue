...
onMounted(async () => {
	...
	apiFetch<Country[]>("/countries").then(c => { countries.value = c; }).catch(() => {});
...
function stepLabel(step: string): string {
	switch (step) {
		case "normalized": return "Address Normalized";
		case "created": return "Master Created";
		case "matched": return "Location Matched";
		case "geocoded": return "Forward Geocoded";
		case "reverse_geocoded": return "Reverse Geocoded";
		case "spatial_matched": return "Spatial Matching";
		case "llm_verified": return "LLM Verified";
		case "edited": return "Address Edited";
...
function stepIcon(step: string): string {
	switch (step) {
		case "normalized": return "pi pi-filter";
		case "created": return "pi pi-plus-circle";
		case "matched": return "pi pi-link";
		case "geocoded": return "pi pi-map-marker";
		case "reverse_geocoded": return "pi pi-replay";
		case "spatial_matched": return "pi pi-sitemap";
		case "llm_verified": return "pi pi-microchip-ai";
    ...
    <template v-else-if="masterLocation">
      ...
      <div class="page-header">
        <div>
          ...
          <p class="page-subtitle">{{ masterLocation.city }}, {{ countryDisplay(masterLocation.country) }}</p>
        </div>
        <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
          <Tag
            :value="masterLocation.status"
            :severity="masterLocation.status === 'VALIDATED' ? 'success' : masterLocation.status === 'GEOCODED' ? 'info' : masterLocation.status === 'PENDING' ? 'warn' : 'danger'"
          />
          <Button
            v-if="!editing"
            label="Edit"
      ...
      <div v-if="editing" class="fc-card" style="margin-bottom: 16px; border: 2px solid #0967d2;">
        ...
        <form @submit.prevent="handleSaveEdit">
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">House Number</label>
              <InputText v-model="editForm.houseNumber" placeholder="e.g. 123" class="w-full" />
            </div>
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">Road</label>
              <InputText v-model="editForm.road" placeholder="e.g. Main Street" class="w-full" />
            </div>
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">Suburb</label>
              <InputText v-model="editForm.suburb" placeholder="e.g. Sandton" class="w-full" />
            </div>
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">City</label>
              <InputText v-model="editForm.city" class="w-full" required />
            </div>
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">State / Province</label>
              <InputText v-model="editForm.state" class="w-full" />
            </div>
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">Postal Code</label>
              <InputText v-model="editForm.postalCode" class="w-full" />
            </div>
            <div>
              <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px;">Country</label>
              <Select
              ...
              >
                <template #value="slotProps">
                  <span v-if="slotProps.value">
                    {{ countries.find(c => c.isoA3 === slotProps.value)?.name }} ({{ slotProps.value }})
                  </span>
                  <span v-else style="color: #94a3b8;">Select country</span>
                </template>
                <template #option="slotProps">
                  {{ slotProps.option.name }} <span style="color: #64748b; font-size: 12px;">({{ slotProps.option.isoA3 }})</span>
                </template>
              </Select>
              <InputText v-else v-model="editForm.country" class="w-full" required placeholder="ISO A3 code (e.g. ZAF)" />
            </div>
          </div>
          <div style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px;">
            <Button label="Cancel" severity="secondary" @click="editing = false" />
            <Button label="Save & Reset to Pending" type="submit" icon="pi pi-save" :loading="saving" />
          </div>
        </form>
      </div>
      ...
      <div v-if="reverseResult" class="fc-card review-panel" style="margin-bottom: 16px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          ...
          <Tag :value="`${(reverseResult.confidence * 100).toFixed(0)}% confidence`" severity="info" />
        </div>
        ...
        <div style="display: flex; gap: 8px; justify-content: flex-end;">
          ...
          <Button label="Confirm & Validate" icon="pi pi-check" :loading="confirming" @click="handleConfirm" />
        </div>
      </div>
      ...
      <div v-if="masterLocation.features.length > 0" class="fc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 12px; font-size: 16px; color: #243b53;">
          ...
          Matched Features
          <Tag :value="String(masterLocation.features.length)" severity="info" style="margin-left: 8px;" />
        </h3>
        <DataTable :value="masterLocation.features" size="small">
          ...
          <Column header="">
            <template #body="{ data }">
              <RouterLink :to="`/layers/${(data as FeatureAssociation).layerId}`" class="detail-link">
                View Layer
              </RouterLink>
            </template>
          </Column>
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item">
            ...
            <span class="detail-value">
              {{ masterLocation.latitude != null ? `${masterLocation.latitude}, ${masterLocation.longitude}` : 'Not geocoded' }}
            </span>
          </div>
          <div class="detail-item">
      ...
      <!-- Map -->
      <div v-if="masterLocation.latitude != null && masterLocation.longitude != null" class="fc-card" style="margin-bottom: 16px;">
      ...
      <!-- Processing Log -->
      <div class="fc-card">
        <div style="display: flex; justify-content: space-between; align-items: center; cursor: pointer;" @click="toggleProcessingLog">
          <h3 style="margin: 0; font-size: 16px; color: #243b53;">
            <i class="pi pi-history" style="margin-right: 8px;"></i>
            Processing Log
          </h3>
          <i :class="showLog ? 'pi pi-chevron-up' : 'pi pi-chevron-down'" style="color: #64748b;"></i>
        </div>

        <div v-if="showLog" style="margin-top: 16px;">
          <ProgressSpinner v-if="loadingLog" style="display: flex; justify-content: center; padding: 24px;" />

          <div v-else-if="processingLog.length === 0" style="text-align: center; padding: 24px; color: #94a3b8;">
