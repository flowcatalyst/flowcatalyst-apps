...
onMounted(() => { loadPartitions(); loadMasterLocations(); });
  ...
  <div class="page-container">
    ...
    <div v-else class="fc-card">
      <div style="display: flex; gap: 12px; margin-bottom: 16px;">
        <InputText
          v-model="searchQuery"
          placeholder="Search master locations..."
          style="flex: 1;"
        />
        <Select
        ...
        <Column header="Coordinates">
          <template #body="{ data }">
            <span v-if="(data as MasterLocation).latitude != null">
              {{ (data as MasterLocation).latitude?.toFixed(4) }}, {{ (data as MasterLocation).longitude?.toFixed(4) }}
            </span>
