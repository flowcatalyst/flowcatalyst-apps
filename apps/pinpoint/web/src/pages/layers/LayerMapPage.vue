...
const layerOptions = computed(() => layers.value.map((l) => ({ label: `${l.name} (${l.code})`, value: l.id })));
...
onMounted(async () => {
	if (!clientId.value) { loading.value = false; return; }
	try {
    ...
    <div class="map-body">
      ...
      <div v-if="selectedFeature" class="feature-panel">
        <div class="panel-header">
          <h3 style="margin: 0; font-size: 15px;">{{ editing ? 'Edit Feature' : selectedFeature.label }}</h3>
          <div style="display: flex; gap: 4px;">
            <Button v-if="!editing" icon="pi pi-pencil" severity="secondary" text size="small" @click="startEdit" />
        ...
        <div v-else class="panel-content">
          <div v-if="selectedFeature.centerLat != null" class="detail-row">
            ...
            <span class="detail-val">{{ selectedFeature.centerLat.toFixed(5) }}, {{ selectedFeature.centerLon?.toFixed(5) }}</span>
          </div>
