...
interface PropertyItem { key: string; value: string; }
interface PropertySetItem { id: string; name: string; description: string | null; properties: PropertyItem[]; }
...
async function handleSavePartitions() {
	...
	try {
		...
		toast.success("Updated", selectedPartitionIds.value.length === 0
...
async function handleSave() {
	...
	try {
		...
		editing.value = false;
	} catch (e) { toast.error("Failed to save", getErrorMessage(e, "Unknown error")); }
...
function confirmDeleteLayer() {
	confirm.require({
		...
		header: "Delete Layer", icon: "pi pi-exclamation-triangle", acceptClass: "p-button-danger",
		accept: async () => {
			try {
				await apiFetch(layerPath(), { method: "DELETE" });
				toast.success("Deleted", "Layer deleted.");
				await router.push("/layers");
			} catch (e) { toast.error("Failed to delete", getErrorMessage(e, "Unknown error")); }
		},
	});
}
...
async function handleSaveFeature() {
	...
	try {
		if (editingFeature.value) {
			...
			const created = await apiFetch<FeatureItem>(
				layerPath("/features"),
				{ method: "POST", body: JSON.stringify(featureForm.value) },
			);
	...
	} catch (e) { featureError.value = getErrorMessage(e, "Failed to save feature."); }
...
function confirmDeleteFeature(f: FeatureItem) {
	confirm.require({
		...
		header: "Delete Feature", icon: "pi pi-exclamation-triangle", acceptClass: "p-button-danger",
		accept: async () => {
			try {
...
async function createSchema() {
	...
	} catch (e) { toast.error("Failed to create schema", getErrorMessage(e, "Unknown error")); }
	finally { creatingSchemaPending.value = false; }
}
...
function addSchemaKey() {
	const ps = propertySet.value;
	if (!ps) return;
	if (ps.properties.length >= 6) { toast.warn("Limit", "Max 6 properties."); return; }
...
async function saveSchema() {
	...
	try {
		await apiFetch(layerPath(`/property-sets/${ps.id}/properties`), {
			method: "PUT", body: JSON.stringify({ properties: ps.properties.filter((p) => p.key.trim()) }),
		});
  ...
  <div class="page-container" style="max-width: 1000px;">
    ...
    <template v-else-if="layer">
      <div style="margin-bottom: 12px;">
        <Button label="Back to Layers" icon="pi pi-arrow-left" severity="secondary" text @click="router.push('/layers')" />
      </div>

      <div class="page-header">
        <div>
          <h1 class="page-title">{{ layer.name }}</h1>
          <p class="page-subtitle">{{ layer.code }} &middot; {{ layer.layerType }}{{ layer.description ? ` — ${layer.description}` : '' }}</p>
        </div>
        <div style="display: flex; gap: 8px;">
          <Button v-if="!editing" label="Edit" icon="pi pi-pencil" severity="secondary" @click="startEdit" />
          <Button v-if="!editing" label="Delete" icon="pi pi-trash" severity="danger" @click="confirmDeleteLayer" />
        </div>
      </div>
      ...
      <div class="fc-card" style="margin-bottom: 16px;">
        ...
        <div v-if="!propertySet" style="text-align: center; padding: 16px;">
          <p style="color: #94a3b8; margin-bottom: 12px;">Define what data each feature in this layer carries (up to 6 columns).</p>
          <Button label="Define Properties" icon="pi pi-plus" severity="secondary" :loading="creatingSchemaPending" @click="createSchema" />
        </div>

        <template v-else>
          <div class="properties-table">
            <div v-for="(prop, idx) in propertySet.properties" :key="idx" class="property-row">
              <InputText v-model="prop.key" placeholder="Column name (e.g. zone, priority)" style="flex: 1;" size="small" />
              <Button icon="pi pi-times" severity="danger" text size="small" @click="removeSchemaKey(idx)" />
            </div>
          </div>
          <div style="display: flex; justify-content: space-between; margin-top: 8px;">
            <Button v-if="propertySet.properties.length < 6" label="Add Column" icon="pi pi-plus" text size="small" @click="addSchemaKey" />
      ...
      <div v-if="partitions.length > 0" class="fc-card" style="margin-bottom: 16px;">
        <h3 style="margin: 0 0 12px; font-size: 16px; color: #243b53;">
          ...
          Partition Scope
          <Tag
            :value="selectedPartitionIds.length === 0 ? 'All Partitions' : `${selectedPartitionIds.length} Partition(s)`"
            :severity="selectedPartitionIds.length === 0 ? 'success' : 'info'"
            style="margin-left: 8px;"
          />
        </h3>
        <p style="font-size: 13px; color: #64748b; margin: 0 0 12px;">
          Leave empty for this layer to apply to all partitions. Select specific partitions to restrict.
        </p>
      ...
      <div class="fc-card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          <h3 style="margin: 0; font-size: 16px; color: #243b53;">
            Features <span style="color: #94a3b8; font-weight: normal;">({{ features.length }})</span>
          </h3>
        ...
        <DataTable v-else :value="filteredFeatures" :rows="20" :paginator="filteredFeatures.length > 20">
          ...
          <Column header="Geometry">
            <template #body="{ data }">
              <span v-if="(data as FeatureItem).centerLat != null">
                {{ (data as FeatureItem).centerLat?.toFixed(4) }}, {{ (data as FeatureItem).centerLon?.toFixed(4) }}
                <span v-if="layer.layerType === 'RADIUS' && (data as FeatureItem).radiusMeters"> ({{ (data as FeatureItem).radiusMeters }}m)</span>
                <span v-else-if="layer.layerType === 'POINT'" style="color: #64748b;"> Point</span>
              </span>
              <span v-else-if="(data as FeatureItem).polygonGeojson" style="color: #64748b;">Polygon</span>
          ...
          <Column header="" style="width: 60px;">
            <template #body="{ data }">
              <Button icon="pi pi-trash" severity="danger" text size="small" @click="confirmDeleteFeature(data as FeatureItem)" />
            </template>
          </Column>
        </DataTable>
      </div>
      ...
      <Dialog v-model:visible="showFeatureDialog" :header="editingFeature ? 'Edit Feature' : 'Add Feature'" :modal="true" style="width: 700px;">
        <div style="display: flex; flex-direction: column; gap: 16px;">
          <div>
            <label style="display: block; margin-bottom: 6px; font-weight: 500;">Label</label>
            <InputText v-model="featureForm.label" placeholder="e.g. Sandton Central" class="w-full" required />
          </div>

          <div v-if="layer.layerType === 'RADIUS'">
            <label style="display: block; margin-bottom: 6px; font-weight: 500;">Radius (meters)</label>
            <InputNumber v-model="featureForm.radiusMeters" :min="1" :max="100000" suffix=" m" class="w-full" />
            <small style="color: #94a3b8;">Layer default: {{ layer.radiusMeters ?? 1000 }}m</small>
          </div>

          <div v-if="layer.layerType === 'POINT'" style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
            <div>
              <label style="display: block; margin-bottom: 6px; font-weight: 500;">Latitude</label>
              <InputNumber v-model="featureForm.centerLat" :min-fraction-digits="1" :max-fraction-digits="8" class="w-full" placeholder="-26.2041" />
            </div>
            <div>
              <label style="display: block; margin-bottom: 6px; font-weight: 500;">Longitude</label>
              <InputNumber v-model="featureForm.centerLon" :min-fraction-digits="1" :max-fraction-digits="8" class="w-full" placeholder="28.0473" />
            </div>
          </div>

          <div>
            <label style="display: block; margin-bottom: 6px; font-weight: 500;">
              {{ layer.layerType === 'POINT' ? 'Click the map to place point (or enter coordinates above)' : layer.layerType === 'RADIUS' ? 'Click the map to set center point' : 'Click the map to draw polygon' }}
            </label>
            <PpLayerEditor
              :mode="layer.layerType === 'POINT' ? 'point' : layer.layerType === 'RADIUS' ? 'radius' : 'polygon'"
          ...
          <div v-if="propertyKeys.length > 0">
            ...
            <div style="display: flex; flex-direction: column; gap: 8px;">
              <div v-for="key in propertyKeys" :key="key" style="display: flex; gap: 8px; align-items: center;">
                <label style="min-width: 120px; font-size: 14px; color: #64748b;">{{ key }}</label>
                <InputText v-model="featureForm.propertyValues[key]" :placeholder="key" style="flex: 1;" />
              </div>
            </div>
          </div>

          <div v-if="featureError" style="padding: 8px 12px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 6px; color: #dc2626; font-size: 14px;">
            {{ featureError }}
          </div>
        </div>
        <template #footer>
          <Button label="Cancel" severity="secondary" @click="showFeatureDialog = false" />
          <Button :label="editingFeature ? 'Update' : 'Create'" :loading="featureSaving" @click="handleSaveFeature" />
        </template>
      </Dialog>
