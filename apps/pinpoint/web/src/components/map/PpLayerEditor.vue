...
function clearDrawing() {
	...
	for (const m of tempMarkers) { map?.removeLayer(m); }
	...
	if (props.mode === "point") {
		...
		if (polygonPoints.value.length >= 2) {
			polygon = L.polygon(polygonPoints.value, {
				color: "#0967d2", weight: 2, fillOpacity: 0.15, dashArray: "5,5",
			}).addTo(map);
		}
	}
}
...
function handleMapDblClick(e: L.LeafletMouseEvent) {
	...
	if (polygonPoints.value.length >= 3) {
		if (polygon) map.removeLayer(polygon);
		polygon = L.polygon(polygonPoints.value, {
			color: "#0967d2", weight: 2, fillOpacity: 0.15,
		}).addTo(map);
...
watch(() => props.mode, () => { initializeMode(); });
