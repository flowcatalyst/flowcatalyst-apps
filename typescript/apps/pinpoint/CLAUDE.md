...
- **Testing**: Vitest — unit tests use fakes; integration tests use real `runWrite` + `@testcontainers/postgresql` + `@testcontainers/redis`. `pnpm typecheck` runs `tsc -p tsconfig.test.json`, which covers `src/`, `test/` and `scripts/` — stale test fakes fail the typecheck, not the test run.
...
├── flows/                             # @pinpoint/flows — flow walkthrough harness (drives a running server end to end)
└── web/                               # Vue 3 SPA
```
...
- **Flow walkthrough (`apps/pinpoint/flows`, `@pinpoint/flows`)**: `pnpm flows:pinpoint` drives a _running_ server end to end through its own generated typed client (`flows/src/schema.gen.d.ts`, regenerated by `pnpm openapi:pinpoint`) — seeds a Cape Town dataset, then narrates create → match (new / FUZZY / EXACT_HASH) → geocode → confirm → feature association → scheduled-job webhook (HMAC) → rematch → operator actions → outbox, with soft assertions (exit 1 on failure). Start the server with `PINPOINT_AUTH_DEV_FALLBACK=true`; `--seed-only` leaves data for the SPA, `--cleanup` removes it, `--base-url` targets any environment (it doubles as a smoke test). See `flows/README.md`.
...
- **Shared schemas → `components.schemas`.** A TypeBox schema with an `$id`, listed in `api/plugins/shared-schemas.ts` (`SHARED_SCHEMAS`, which `server.ts` registers via `addSchema`), becomes a named component; routes reference it through `schemaRef(schema)` (`api/plugins/schema-ref.ts` — a typed `$ref`, so `reply.send` / `request.body` stay checked). Today: `ErrorResponse` (every 4xx/5xx uses `ErrorResponseRef`; never declare a local `ErrorSchema`), `BffClient`, `BffPartition`, `BffLayerDetail` + `BffLayerPropertySet`, `BffLayerFeature` + `BffLayerFeatureInput`, `BffFeatureAssociation`, `BffMasterLocation` (the get route intersects it with `features` → `allOf`), `MatchingConfig`, `RematchLocationBody` / `RematchLocationResponse`. Rule: when two or more operations share a shape, put it in a `*.schema.ts` next to the routes, give it an `$id`, add it to `SHARED_SCHEMAS`, use the `*Ref` in routes. `test/api/shared-schemas.test.ts` guards unique ids + `$ref` serialization. The swagger `refResolver` keeps component names equal to the `$id`.
- Consumers: client/type generation for the Vue SPA and the flows harness (`openapi-typescript`), any non-TS port of the service, and the FlowCatalyst platform.
- **Publishing to the platform**: `pnpm flowcatalyst:sync` also pushes `openapi.gen.json` to `POST /api/applications/pinpoint/openapi/sync` (versioned; reports `unchanged` / `hasBreaking`) and publishes `docs/architecture.md` as five Markdown pages via `…/docs/sync` (Mermaid renders there). `src/flowcatalyst/docs.ts` owns the page split — adding a `## N.` section to architecture.md without mapping it to a page fails the sync loudly. `-- --docs-only` skips phases 1–2; `PINPOINT_DOCS_SYNC=false` skips phase 3.
