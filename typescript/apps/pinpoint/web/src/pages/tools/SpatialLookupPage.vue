  ...
  <div class="page-container" style="max-width: 1100px">
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">
          {{ clientStore.selectedClient?.name ?? 'Select a client' }} — Find layer features at a
          coordinate
        </p>
      </div>
    </div>
    ...
    <template v-else>
      <div class="fc-card" style="margin-bottom: 16px">
        <div
          style="display: grid; grid-template-columns: 1fr 1fr auto; gap: 12px; margin-bottom: 12px"
        >
          <div>
        ...
        <div v-if="layers.length > 0" style="margin-bottom: 12px">
          <label style="display: block; margin-bottom: 4px; font-weight: 500; font-size: 13px"
            >Layers (optional — leave empty for all)</label
          >
          <MultiSelect
        ...
        <div>
          <label style="display: block; margin-bottom: 6px; font-weight: 500; font-size: 13px"
            >Click map to set coordinates</label
          >
          <PpLayerEditor
            mode="point"
            :center-lat="lat ?? -26.2"
      ...
      <div v-if="results.length > 0" class="fc-card">
        ...
        <DataTable :value="results" size="small">
          <Column field="layerName" header="Layer">
            <template #body="{ data }">
              ...
              <span style="color: #94a3b8; font-size: 12px; margin-left: 4px"
                >({{ (data as LookupFeature).layerCode }})</span
              >
            </template>
          </Column>
          ...
          <Column header="Properties">
            <template #body="{ data }">
              <div
                v-for="(val, key) in (data as LookupFeature).properties"
                :key="key"
              ...
              <span
                v-if="Object.keys((data as LookupFeature).properties).length === 0"
                style="color: #94a3b8"
                >—</span
              >
            </template>
