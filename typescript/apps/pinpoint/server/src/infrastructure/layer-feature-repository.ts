...
export function createDrizzleLayerFeatureRepository(
  ...
  return {
    ...
    async findById(id: LayerFeatureId): Promise<LayerFeature | null> {
      const [row] = await db.select().from(layerFeatures).where(eq(layerFeatures.id, id)).limit(1);
    ...
    async findFeatureAssociations(locationId: string): Promise<readonly FeatureAssociation[]> {
      const rows = await db
        .select({
        ...
        .innerJoin(layerFeatures, eq(layerFeatures.id, locationFeatureAssociations.layerFeatureId))
