...
<header class="hero">
  ...
  <div class="meta"><span>As built <b>2026-08-22</b></span><span>Source of truth: <b>apps/pinpoint/docs/architecture.md</b></span><span>Contract: <b>openapi.gen.json</b></span></div>
</header>
...
<section id="platform">
  ...
  <div class="tbl"><table>
    ...
    <tr><td><b>Documentation</b></td><td>pinpoint → platform (script)</td><td>the same script publishes the <b>OpenAPI document</b> (<code>POST /api/applications/pinpoint/openapi/sync</code> — versioned, reports <code>unchanged</code> / <code>hasBreaking</code>) and this document as five <b>Markdown pages</b> (<code>…/docs/sync</code>, Mermaid renders). Readable at <code>GET /api/docs</code>.</td></tr>
...
<section id="data">
  ...
  <p><b>Testing:</b> 189 unit tests (matcher, normaliser, verifier, auth, shared-schema guard) · 127 integration tests (testcontainers PostGIS + Redis, one test per use case through the real <code>runWrite</code>, repositories, OIDC end-to-end against a fake IdP) · typecheck covers <code>src</code>, <code>test</code> and <code>scripts</code>. <b>Flow walkthrough:</b> <code>pnpm flows:pinpoint</code> (<code>apps/pinpoint/flows</code>) drives a running server end to end through the typed client with a seeded Cape Town dataset — create → match → geocode → confirm → feature association → scheduled-job webhook → rematch → operator actions → outbox — narrated with assertions; <code>--seed-only</code> leaves the data for the SPA. Full configuration reference in <code>docs/architecture.md</code> §12.</p>
</section>
...
<section id="gaps">
  ...
  <ul class="gaps">
    ...
    <li><b>Resolved 2026-08-22 (found by the flow walkthrough):</b> feature associations written during create-location / rematch ran outside the use-case transaction and hit the <code>locations</code> FK once boundaries worked — now on the transaction; the libpostal normaliser took a trailing postcode / “City 8001” segment as the <b>country</b> (poisoning hash and match score) and accepted libpostal’s first <code>/expand</code> option (“long st” → “long saint”) — it now rejects consumed trailing segments (so the supplied <code>countryCode</code> retry applies) and prefers expansions that agree with the substitution table / end in a street type.</li>
  </ul>
</section>
</main>
