...
describe('CreateLocationUseCase (integration)', () => {
  ...
  it('no-match path creates a fresh master + location', async () => {
    ...
    // Both rows landed.
    const location = await appContext.repositories.locations.findById(data.locationId as never);
  ...
  it('exact-hash-match against a VALIDATED master reuses it', async () => {
    ...
    const pending = await appContext.repositories.masterLocations.findById(firstMasterId as never);
