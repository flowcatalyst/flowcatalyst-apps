...
describe('UpdateMatchingConfigUseCase (integration)', () => {
  ...
  it('promotes a scoped config from the global default + emits the event', async () => {
    ...
    const config = await appContext.repositories.matchingConfigs.resolve(clientId as never, null);
  ...
  it('reuses the scoped row on a second update', async () => {
    ...
    const after1 = await appContext.repositories.matchingConfigs.resolve(clientId as never, null);
    ...
    const after2 = await appContext.repositories.matchingConfigs.resolve(clientId as never, null);
