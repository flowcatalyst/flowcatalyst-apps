...
> Spatial geocoding + address-matching service for the FlowCatalyst platform. This document describes the system as built in `apps/pinpoint/` on 2026-08-22; the pickup notes in [HANDOFF.md](HANDOFF.md) and the coding conventions in [../CLAUDE.md](../CLAUDE.md) complement it. Diagrams are Mermaid — GitHub renders them inline.
...
**Contract pipeline.** Every route carries a TypeBox schema and a unique `operationId`; `@fastify/swagger` derives the OpenAPI document, `pnpm openapi:pinpoint` exports it to `apps/pinpoint/openapi.gen.json` and regenerates `web/src/api/schema.gen.d.ts` (`openapi-typescript`); `pnpm openapi:pinpoint:check` fails CI-style when either is stale. Shared shapes are `$id` schemas listed in `api/plugins/shared-schemas.ts` and become `components.schemas` (13 today: `ErrorResponse`, `BffClient`, `BffPartition`, `BffLayerDetail`, `BffLayerPropertySet`, `BffLayerFeature`, `BffLayerFeatureInput`, `BffFeatureAssociation`, `BffLocationSummary`, `BffMasterLocation`, `MatchingConfig`, `RematchLocationBody`, `RematchLocationResponse`). The SPA consumes the contract through `web/src/api/client.ts` (`api` = openapi-fetch, `ok()`, `ApiResponse<>`); there is deliberately no untyped escape hatch.
...
| **Documentation**    | pinpoint → platform (script)  | the same script publishes the **OpenAPI document** (`POST /api/applications/pinpoint/openapi/sync` — the platform versions it and reports `unchanged` / `hasBreaking`) and this document as five **Markdown pages** (`…/docs/sync`, where Mermaid fences render). Readable at `GET /api/docs` and `/api/docs/applications/pinpoint/{slug}`. `-- --docs-only` pushes just these; `PINPOINT_DOCS_SYNC=false` skips them.         |
| **Subscriptions**    | —                             | none today (`subscriptions.ts` returns `[]`); pinpoint publishes but does not consume platform events.                                                                                                                                                                                                                                                                                                                         |

State on **fc-dev**: 33 pinpoint event types, each with schema version `1.0.0`; scheduled job `ACTIVE`; OpenAPI spec version `0.0.1` `CURRENT`; five docs pages. A re-run reports `skipped=33` and `openapi: unchanged`.
...
- **Contract** `pnpm openapi:pinpoint` / `:check` (server spec → web types → flows types); **platform** `pnpm flowcatalyst:sync` (definitions + event schemas + the OpenAPI spec + these docs pages; `-- --docs-only` for the last two); **DB** `db:init`, `db:generate`, `db:migrate`; **dev** `pnpm dev:pinpoint` (server `:3100` + vite `:5173` proxying `/bff /api /auth /jobs`).
