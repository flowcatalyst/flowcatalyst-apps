 ...
 * (`depot_ref` — the depots registry; a depot serves MANY stores via
 * depot_stores). Credentials are stored ONLY as scrypt hashes (`pin_hash`).
 * Staff code is unique within a depot so PIN login can resolve the driver
 * from (depot, staffCode).
 */
export const driverUsers = pgTable(
  'driver_users',
  {
    ...
    defaultVehicleClass: text('default_vehicle_class'),
  ...
  (t) => [uniqueIndex('uq_driver_users_staff_code').on(t.clientId, t.depotRef, t.staffCode)],
);
