/**
 * @fulfil-go/framework — single-import surface for fulfil-go server code.
 *
 ...
 * it directly. fulfil-go's whole surface is plain async/await, like pinpoint.
 */
...
// un-prefixed names since there's no Effect path to clash with in fulfil-go.
// Fulfil keeps importing the `plain*`-prefixed names directly.
export {
...
// from a single `@fulfil-go/framework` entry.
export {
  Scope,
  ScopeStore,
  TransactionStore,
...
// fulfil-go additions on top of the pinpoint re-export surface.
export { brandedTsid, isTsid, asTsid } from './tsid.js';
